  ...
  app.post('/v1/search', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    while (retryCount <= MAX_RETRIES) {
      ...
      try {
        ...
        logProxy(selected, requestedModel, 'success', upstream.status, latency, null, retryCount, downstreamApiKeyId);
        return reply.code(upstream.status).send(data);
      } catch (error: any) {
        tokenRouter.recordFailure(selected.channel.id);
        logProxy(selected, requestedModel, 'failed', 0, Date.now() - startTime, error?.message || 'network error', retryCount, downstreamApiKeyId);
